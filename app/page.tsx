...
export default function Home() {
  ...
  const [buyResults, setBuyResults] = useState<{ html: string } | null>(null);
  ...
  const [sellResults, setSellResults] = useState<{ html: string } | null>(null);
  const [sellLoading, setSellLoading] = useState(false);

  const [swapAmount, setSwapAmount] = useState('0.1');
  const [swapFrom, setSwapFrom] = useState('ETH');
  const [swapTo, setSwapTo] = useState('USDC');
  const [swapResults, setSwapResults] = useState<{ html: string } | null>(null);
  ...
  function buildQuoteList(quotes: any[], bestQuote: any, responseTime: number) {
    ...
    quotes.forEach(q => {
      ...
      const decimals = ['USDC','USDT','DAI'].includes(q.toCurrency) ? 2 : 8;
  ...
  function buildMiniList(quotes: any[], unit: string, decimals: number) {
    ...
    return quotes.slice(0, 5).map(q => `
      <div class="quote-row">
        <span>${q.provider.split(':')[0]}</span>
        <span>${q.toAmount.toFixed(decimals)} ${unit}</span>
      </div>
    `).join('');
  }
  ...
  async function executeBuy() {
    ...
    try {
      ...
      setBuyResults({ html: buildQuoteList(data.quotes, data.bestQuote, data.responseTimeMs) });
    } catch {
      setBuyResults({ html: '<p class="response-info" style="color:#f85149;">Failed to load quotes</p>' });
    } finally {
      setBuyLoading(false);
    }
  }
  ...
  async function executeSell() {
    ...
    try {
      ...
      setSellResults({ html: buildQuoteList(data.quotes, data.bestQuote, data.responseTimeMs) });
    } catch {
      setSellResults({ html: '<p class="response-info" style="color:#f85149;">Failed to load quotes</p>' });
    } finally {
  ...
  async function executeSwap() {
    ...
    try {
      ...
      setSwapResults({ html: buildQuoteList(data.quotes, data.bestQuote, data.responseTimeMs) });
    } catch {
      setSwapResults({ html: '<p class="response-info" style="color:#f85149;">Failed to load quotes</p>' });
    } finally {
      ...
      setSpreadData(`
        <div class="spread-grid">
          <div class="spread-col">
            <div class="spread-title">💰 BUY $35 → BTC</div>
            ${buildMiniList(buy.quotes, 'BTC', 8)}
          </div>
          <div class="spread-col">
            <div class="spread-title">💸 SELL 0.0012 BTC → USD</div>
            ${buildMiniList(sell.quotes, 'USD', 2)}
          </div>
          <div class="spread-col">
            <div class="spread-title">🔄 SWAP 0.1 ETH → USDC</div>
            ${buildMiniList(swap.quotes, 'USDC', 2)}
          </div>
        </div>
      `);
    } catch {
      setSpreadData('<p class="response-info" style="color:#f85149;">Failed to load spreads</p>');
    } finally {
    ...
    <div className="phone-app">
      ...
      <div className="wallet-header">
        ...
        <div className="wallet-balance">
          <div className="balance-amount">$12,847.32</div>
          ...
          <>
            <div className="trade-form">
              ...
              <div className="trade-row">
                <select className="trade-select" value={buyFiat} onChange={e => setBuyFiat(e.target.value)}>
                  <option>USD</option><option>EUR</option><option>GBP</option>
                </select>
                <span className="trade-arrow">→</span>
                <select className="trade-select" value={buyCrypto} onChange={e => setBuyCrypto(e.target.value)}>
            ...
            {buyResults && <div dangerouslySetInnerHTML={{ __html: buyResults.html }} />}
          </>
        )}
        ...
        {tab === 'sell' && (
          <>
            <div className="trade-form">
              ...
              <div className="trade-row">
                <select className="trade-select" value={sellCrypto} onChange={e => setSellCrypto(e.target.value)}>
                  <option>BTC</option><option>ETH</option><option>USDC</option><option>USDT</option>
                </select>
                <span className="trade-arrow">→</span>
                <select className="trade-select" value={sellFiat} onChange={e => setSellFiat(e.target.value)}>
                  <option>USD</option><option>EUR</option><option>GBP</option>
                </select>
              </div>
            ...
            {sellResults && <div dangerouslySetInnerHTML={{ __html: sellResults.html }} />}
          </>
        )}
        ...
        {tab === 'swap' && (
          <>
            <div className="trade-form">
              ...
              <div className="trade-row">
                <select className="trade-select" value={swapFrom} onChange={e => setSwapFrom(e.target.value)}>
                  <option>ETH</option><option>BTC</option><option>USDC</option><option>USDT</option><option>MATIC</option>
                </select>
                <span className="trade-arrow">→</span>
                <select className="trade-select" value={swapTo} onChange={e => setSwapTo(e.target.value)}>
                  <option>USDC</option><option>USDT</option><option>ETH</option><option>BTC</option><option>DAI</option>
                </select>
              </div>
            ...
            {swapResults && <div dangerouslySetInnerHTML={{ __html: swapResults.html }} />}
          </>
        )}
